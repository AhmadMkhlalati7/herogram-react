...
const Game = ({sudoku}) => {
            ...
            if (sudokuIsRight()) {
                ...
                if (localStorage.getItem("sudokuUser")) {
                    dispatch(updateStatistics({
                        mode,
                        user: localStorage.getItem("sudokuUser"),
                        timeElapsed
                    }));
                }
            } else {
    ...
    const formatSecondToTime = (timeInSeconds) => {
  ...
  {sudoku?.bestTime ? (
    <>
      The best player Solve this mode with time : {formatSecondToTime(sudoku.bestTime)}
    </>
  ) : (
    <span>No best player data available.</span>
  )}
</div>                        <div className="main-sudoku-grid">
                ...
                <div className="text">You won in {formatTime(timeElapsed)}!</div>
